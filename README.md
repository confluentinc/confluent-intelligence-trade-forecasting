...
You'll stand up two **Sample Data** (Datagen) connectors that stream mock records into Kafka topics: **Users** (customer profiles — `userid`, `regionid`, `gender`) and **Stock trades** (live trades — `symbol`, `side`, `quantity`, `price`, and the trader's `userid`).
